...
class evalPretrainedTransformers:

    def eval_pretrained_transformers(self, elasticRetriever, eval_file, lang, models, pooling_methods, extraction_layers):
        for model_name in models:
            for pooling_method in pooling_methods:
                for extraction_layer in extraction_layers:
                    ...
                    # pass elasticRetriever to etriever
                    ...
                    evalQuesSim.eval_question_similarity(y_true=y_true, y_pred=df["pred"].values, lang=lang, model_name=model_name,
...
#    models = ["deepset/sentence_bert","bert-base-uncased", "DeepPavlov/bert-base-multilingual-cased-sentence"]
    ...
    # initialize Elastic_Search retriever outside first and pass it
    if models:
        for model_name in models:
            for pooling_method in pooling_methods:
                for extraction_layer in extraction_layers:
        ...
        model_name = None
    elasticRetriever = ElasticsearchRetriever(document_store=None, embedding_model=model_name, gpu=True)

    evalPreTrans = evalPretrainedTransformers()
    evalPreTrans.eval_pretrained_transformers(elasticRetriever = elasticRetriever, eval_file = eval_file, lang=lang, models = models,
    pooling_methods = pooling_methods, extraction_layers = extraction_layers)