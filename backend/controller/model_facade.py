...
###############################################################################
# Structural Design Pattern - Facade is applied  on this file. 
###############################################################################
# There is one condition that needs to be satisfied in order to use the COIVD-QA webpage. 
# The question typed on the search bar must be recognized so that the system can scheme through NLP server
# and find the most suitable answer to the question. 
# Language, is the condition. The questions in English format are most likely processed
# but the questions in Non-English are restricted. 
# Passing the gatekeeper, langauge barrier, the user can have fully access to the webpage and
# the webpage itself can also run its server to produce outcomes. 
# How the coding is running on this file is similar to the Facade pattern. 
# The Facade pattern hides the complexities of the system and provides an interface to the client
# using which the client can access the system. The pattern also adds an interface to existing systems to hide its complexities. 
# The definition of the Facade design pattern matches the intention of programmers who participated on this file. 